...
public class GestionarApoderados extends JDialog implements ActionListener, MouseListener, MouseMotionListener {
	...
	public static int codigoapoderadoseleccionado;
	...
	public static void main(String[] args) {
		try {
			GestionarApoderados dialog = new GestionarApoderados();
	 ...
	 * Create the dialog.
	 */
	public GestionarApoderados() {
		setUndecorated(true);
		setIconImage(Toolkit.getDefaultToolkit().getImage(GestionarApoderados.class.getResource("/imagenes/icon-window.png")));
		setTitle("Gestionar Apoderados");
		...
		lblTituloVentana = new JLabel("Gestionar Apoderados - Sesión de " + Login.user);
		...
		tbApoderados = new JTable();
		tbApoderados.addMouseListener(this);
		scpScroll.setViewportView(tbApoderados);
		
		btnVerApoderado = new JButton("Ver Apoderado");
		btnVerApoderado.addActionListener(this);
		btnVerApoderado.addMouseListener(this);
		btnVerApoderado.setBorder(new LineBorder(Color.GREEN, 2));
		...
		btnVerApoderado.setBounds(400, 385, 150, 30);
		getContentPane().add(btnVerApoderado);
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(e.getSource() == tbApoderados) {
			this.mouseClickedTbApoderados(e);
		}
	}
	public void mouseEntered(MouseEvent e) {
		...
		if(e.getSource() == btnVerApoderado) {
			this.mouseEnteredBtnVerApoderado();;
		}
	}
	...
	public void mouseExited(MouseEvent e) {
		...
		if(e.getSource() == btnVerApoderado) {
			this.mouseExitedBtnVerApoderado();
		}
	}
	...
	private void actionPerformedBtnVerApoderado() {
		if(seleccionregistro) {
			new InformacionApoderado().setVisible(true);
			this.dispose();
		} else {
	...
	private void mouseEnteredBtnVerApoderado() {
		btnVerApoderado.setBorder(new LineBorder(new Color(255, 255, 255), 2));
		btnVerApoderado.setBackground(Color.GREEN);
		btnVerApoderado.setForeground(Color.BLACK);
		btnVerApoderado.setText("VER APODERADO");
	}
	...
	private void mouseExitedBtnVerApoderado() {
	...
	private void mouseClickedTbApoderados(MouseEvent e) {
		int filapos = tbApoderados.rowAtPoint(e.getPoint());
		int columnapos = 0;
		
		if(filapos > -1) {
			codigoapoderadoseleccionado = (int) model.getValueAt(filapos, columnapos);
			seleccionregistro = true;
		}
	}
	
	/*metodo listar apoderados*/
	private void listarApoderados() {
		model.setRowCount(0);
		for(int i = 0; i < objApo.tamanioApoderado(); i++) {
			Object fila[] = {
				objApo.obtenerApoderado(i).getCodigoapoderado(),
				objApo.obtenerApoderado(i).getNombreapoderado(),
				objApo.obtenerApoderado(i).getApellidoapoderado(),
				objApo.obtenerApoderado(i).getDniapoderado(),
				objApo.obtenerApoderado(i).getTelefonoapoderado()
			};
			model.addRow(fila);
		}
	}
	...
	private void ajustarAnchoColumnas() {
		TableColumnModel tcm = tbApoderados.getColumnModel();
		...
		tcm.getColumn(3).setPreferredWidth(this.anchoColumna(8)); //dni
		tcm.getColumn(4).setPreferredWidth(this.anchoColumna(10)); //telefono
	}
